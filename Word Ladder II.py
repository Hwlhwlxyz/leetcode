...
def findLadders(beginWord, endWord, wordList):
    ...
    def find(bword, eword, wlist):
        #print(bword, eword, wlist, oneletter(bword, eword))
        if oneletter(bword, eword):
            return [eword], 1
        minresult = [], 9999
        for w in wlist:
            if oneletter(bword, w):
                copylist = wlist.copy()
                copylist.remove(w)
                rtmp = find(w, eword, copylist)
                l ,c = rtmp
                if l:
                    l.append(w)
                    nrtmp = l, c+1
    ...
    r = find(beginWord, endWord, wordList)
    print(r)
    if r == ([], 9999):
        return 0
    return r[1] + 1
...
def findLadders2(beginWord, endWord, wordList):
    ...
    if endWord not in wordList:
        return 0
    bfslist = [{"beginWord": beginWord, "wordList": wordList, "count": 1}]




    for d in bfslist:
        wordListcopy = d["wordList"].copy()
        #print(d["beginWord"])
        #print(d["wordList"])
        #wordListcopy.remove(d["beginWord"])
        for x in wordListcopy:
            #print(x)
            if oneletter(d["beginWord"], x):
                if x == endWord:
                    #print(d)
                    return d["count"] + 1
                newWordList = wordListcopy.copy()
                newWordList.remove(x)
                bfslist.append({"beginWord": x, "wordList": newWordList, "count": d["count"]+1})
...
#print(findLadders("dog", "cog", ['lot', 'log', 'cog']))
#findLadders("dot", "cog", ['dog', 'lot', 'log', 'cog'] )
#print(findLadders("hot","dog",["hot","dog"]))
#print(findLadders("a","c",["a","b","c"]))

#x= findLadders("hot","dog",["hot","cog","dog","tot","hog","hop","pot","dot"])
#print(findLadders("hot","dog",["hot","dog"]))
'''
def findLadders(beginWord, endWord, wordList):
    def oneletter(w1, w2):
    ...
    for i in range((len(wordList))):
        for j in range(i+1, len(wordList)):
            if oneletter(wordList[i], wordList[j]):
                if wordList[i] not in oneletterdict:
                    oneletterdict[wordList[i]] = [wordList[j]]
                else:
                    oneletterdict[wordList[i]].append(wordList[j])
                if wordList[j] not in oneletterdict:
                    oneletterdict[wordList[j]] = [wordList[i]]
                else:
                    oneletterdict[wordList[j]].append(wordList[i])

    bfslist = [(beginWord, [beginWord])]
    for x in bfslist:
        word, transferlist = x
        for i in oneletterdict[word]:
            if i == endWord:
                return transferlist
            if i not in transferlist:
                bfslist.append((i, transferlist+[i]))
                #print((i, transferlist+[i]))
        ...
        for j in range(i+1, len(wordList)):
            if oneletter(wordList[i], wordList[j]):
                if wordList[i] not in oneletterdict:
                    ...
                    oneletterdict[wordList[i]].add(wordList[j])
                if wordList[j] not in oneletterdict:
                    oneletterdict[wordList[j]] = {wordList[i]}
                else:
                    oneletterdict[wordList[j]].add(wordList[i])

    bfslist = [(beginWord, [beginWord])]
    alphabet = "abcdefghijklmnopqrstuvwxyz"
    for x in bfslist:
        word, transferlist = x
        for newword in oneletterdict[word]:
            if newword == endWord:
                print(transferlist)
                return len(transferlist)+1
            if newword not in transferlist:
                bfslist.append((newword, transferlist+[newword]))
    ...
    for x in bfslist:
        ...
        for i, l in enumerate(word):
            for t in alphabet:
                ...
                if t != l:
                    newword = part1 + t + part2

                    if newword == endWord:
                        print(translist, newword)


                    #if newword in wordListset and newword not in translist:

                    if newword in wordListset and newword not in translist:
                        newtranslist = translist.copy()
                        tmpx = newword
                        newtranslist.add(newword)
                        #print(newword, newtranslist)
                        bfslist.append((newword, newtranslist))
        if word in wordListset:
            wordListset.remove(word) #删除已经使用过的单词
    return 0


######################别人参考
def findLaddersOthertest(begin, end, wordlist):
    ...
    def findLaddersx(start, end, dict):
    ...
    return findLaddersx(begin, end, wordlist)
import collections
import string
def ladderLength2(start, end, arr):
